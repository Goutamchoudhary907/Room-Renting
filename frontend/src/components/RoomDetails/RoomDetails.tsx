...
interface RoomDetailsData extends RoomFormData {
  images?: Image[];
}

interface ShortTermTotal{
  subTotal:number;
...
const [leaseDuration, setLeaseDuration] = useState<number>(1); //Default one month 
  ...
  const calculateShortTermTotal = ():ShortTermTotal => {
    ...
    const serviceCharge=Math.ceil(subTotal * 0.05);    // 5% service charge
    return{
      subTotal,
      serviceCharge,
  ...
  <div className="mt-8">
    <h4 className="text-xl font-bold mb-4">What this place offer</h4>
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
      {property.amenities.map((amenityValue) => {
        const amenity = amenityOptions.find(opt => opt.value === amenityValue);
        return amenity ? (
          <div key={amenity.value} className="flex items-center gap-3 p-3  rounded-lg">
            <img 
    ...
    {property.rentalType === 'both' ? (
      <div className="flex justify-between items-center">
        <div className="flex items-end">
          <p className="font-bold text-2xl text-black mr-2">₹{property.pricePerNight}</p>
          <p className="text-[#4B5563] font-semibold">/night</p>
        </div>
        <div className="flex items-end">
          <p className="font-bold text-2xl text-black mr-2">₹{property.pricePerMonth}</p>
  ...
  {property.rentalType === 'both' && (
    <div className="flex gap-2 mb-4">
      <button
        ...
        className={`py-2 px-4 rounded-lg flex-1 ${rentalType === 'short-term' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
      >
        Short Stay
      </button>
      <button
        onClick={() => setRentalType('long-term')}
        className={`py-2 px-4 rounded-lg flex-1 ${rentalType === 'long-term' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
      >
        Long Lease
      </button>
    </div>
  ...
  <div className="space-y-4 mb-6">
    ...
    <div className="text-xs text-gray-500 mt-2">
      <p>You can adjust the lease duration later with the property owner</p>
    </div>
  </>
)}
  </div>
 {/* Price Calculation and Total */}
  ...
  <div className="flex justify-between font-bold text-lg">
    <span>{rentalType === 'short-term' ? 'Total' : 'Due Now'}</span>
    <span>
      ₹{rentalType === 'short-term' ? calculateShortTermTotal().total : property.pricePerMonth}
    </span>
  </div>
     </div>
